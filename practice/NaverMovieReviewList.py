...
for i, one in enumerate(review_list):
    ...
    idw_end = original_writer.find('(')
    writer = original_writer[0:idw_end]

    # 날짜 정보 수집
    original_date = one.select('div.score_reple dt em')[1].get_text()

    time_end = original_date.find(' ')
    date = original_date[:time_end]
    # date = original_date[:10]

    print(':: REVIEW -> {}'.format(review))
    print(':: DATE -> {}'.format(writer))
    print(':: SCORE -> {}'.format(score))
    print(':: WRITER -> {}'.format(date))